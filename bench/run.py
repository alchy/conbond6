...
def run_doc(doc: Doc, oracle: CachedOracle, *, strop: int = 0, twice: bool = False,
    ...
    if audit_n and audit_dir is not None:
        # seed = jméno dokumentu (ne commit): tytéž věty ve vzorku napříč commity
        audit = run_audit(m, doc.name, judge, audit_dir / f"audit-{doc.name}.json", audit_n, doc.name, topic=doc.topic)
